...
class HBaseWriter extends RichSinkFunction[String]{
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    config.set("hbase.zookeeper.quorum", "172.16.122.54")
    config.set("hbase.zookeeper.property.clientPort", "2181")
    config.set("hbase.security.authentication.tbds.secureid", "dPRH5zhmsETx2Qpl7YMWIkQ8mgwXY9mTMtLw")
    config.set("hbase.security.authentication.tbds.securekey", "GvGv1VJ3QdqKUUJGw1Ghxeq8gMltafNI")
    config.set("zookeeper.znode.parent", "/hbase-unsecure")
    config.setInt(HConstants.HBASE_CLIENT_OPERATION_TIMEOUT, 30000)
    config.setInt(HConstants.HBASE_CLIENT_SCANNER_TIMEOUT_PERIOD, 30000)
    conn = ConnectionFactory.createConnection(config)

    val tableName: TableName = TableName.valueOf("lx_hbase:tableName")
